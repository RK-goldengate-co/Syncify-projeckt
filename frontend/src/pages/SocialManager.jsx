...
export default function SocialManager() {
  ...
  const addAccount = () => {
    ...
    // Duplicate checks
    const nameExists = accounts.some(a => (a.name || '').toLowerCase() === name.trim().toLowerCase());
          ...
          <div className="grid gap-4">
            {(accounts
              .filter(acc => {
                ...
                const okSearch = !s || acc.name.toLowerCase().includes(s) || (acc.url || '').toLowerCase().includes(s);
                const okTag = !filterTag || (acc.tag || '').toLowerCase().includes(filterTag.toLowerCase());
                const okPlatform = !platformFilter || (acc.platform || '').toLowerCase() === platformFilter.toLowerCase();
                const okConnected = !connectedFilter || (connectedFilter === 'connected' ? acc.connected : !acc.connected);
                return okSearch && okTag && okPlatform && okConnected;
              })
              .sort((a,b) => {
                const dir = sortDir === 'asc' ? 1 : -1;
                if (sortKey === 'name') return a.name.localeCompare(b.name) * dir;
                if (sortKey === 'posts') return ((a.posts||0)-(b.posts||0)) * dir;
                // createdAt default
                ...
                <div className="mt-4 pt-4 border-t border-gray-100">
                  <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
                    ...
                    <div className="text-sm text-gray-600">
                      {t.schedule}: {schedules[idx]?.length > 0 ? schedules[idx].join(', ') : 'Chưa có lịch'}
                    </div>
                  </div>
                </div>
              </div>
